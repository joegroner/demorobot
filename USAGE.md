...
**DemoRobot allows you to turn your software demo in a fully tailored demo for your prospect in minutes:
You simply provide text & visual replacements for your application's UI and DemoRobot turns your generic demo
into a personalized experience for your audience.**

**Curious how this works? Read on to learn how you can use DemoRobot, to tamper your web application to demo almost anything.**
...
DemoRobot is driven by configuration files you write in the integrated editor. You provide lists of search and replace
patterns, that will be applied once DemoRobot is running. The structure of the file is similar to an **ini** file:
...
Options allow you to change the behavior of DemoRobot. Since you might have multiple options of the same
...
```ini
; The configuration only works on sites that include demorobot
@include[] = /demorobot/
; The configuration does not work on demorobot.net
@exclude[] = /demorobot.net/ 
```
...
```ini
@namespace[] = demorobot
; without the line above you would have to write !datarobot.hideApplication(test) below
!hideApplication(test)
```

NOTE: this is an example only. The demorobot namespace doesn't currently exist.

### Block- and Allowlist tags

By default DemoRobot is applied on all tags within the document except `script` and `style`. You can change this by
...
In some special cases, you need to replace text elements, that are not captured by the default mechanisms of DemoRobot.
...
You can provide tags in your configuration, to improve DemoRobot's search:
...
```ini
@author[] = Demo Robot <demorobot@demorobot.net>
```
...
By typing `%` you get a list of all your configurations. Select or type the name of one of them and DemoRobot will copy
...
```ini
!replace(DemoRobot, /usage.html, h1 > a.external-link, title) = TestMonkey
```
...
For example, if you have a link `<a href="http://www.demorobot.net">DemoRobot</a>` and you want to replace the href
attribute you can do the following:

```ini
!replaceAttribute(DemoRobot, href) = https://www.demorobotreplacement.com
```
...
```ini
!protect(DemoRobot)
; the following replacement will not be applied on every instance of "DemoRobot"
Demo = Test
```
...
You can hook into web requests with DemoRobot, so that you can either replace the target url, block the request or
introduce some delay before the request is completed.
Since this set of commands requires higher privileges from your browser, you need to enable DemoRobot explicitly to
...
!delayUrl(http://api.demorobot.net/v1/*) = 5
...
Some but not all commands of DemoRobot allow you to restrict application of the command based on CSS selectors or URL
location. The `if` command is a generic
approach to this:

```ini
!if(home.html, h1.title, DemoRobot) = Example
```

This will replace `DemoRobot` with `Example` only if the URL contains `home.html` and if the css of the parent node
...
!if(,h1.title,!replace(DemoRobot)) = Example
!ifSelector(h1.title,!replace(DemoRobot)) = Example
```
...
```ini
; change the font color of every occurrence of DemoRobot to red.
!style(DemoRobot, color) = red
```
...
If you need to add a small piece of JavaScript into a website, you can do this with DemoRobot:
...
DemoRobot is not a perfect match for more complex use cases. There are multiple chrome extensions that do a better job,
if needed.
...
```ini
!insertBefore(DemoRobot, 2, index.html) = $beforeHtml
!insertAfter(DemoRobot, 2, index.html) = $afterHtml
```
...
!replaceAjaxResponse(/v1/items) = '[{"name": "Demo"}, {"name": "Robot"}]'
; apply a search and replace on the response, 
; here every occurrence of "Example" is replaced with "DemoRobot"
!replaceAjaxResponse(/v1/items, Example) = DemoRobot
; apply a json patch on the given response
!patchAjaxResponse(/v1/items) = [{"op": "replace", "path": "/0/name", "value": "DemoRobot"}]
```
...
If you have a set of common demo flows you use regularly, DemoRobot can help you to understand how long it takes
...
While going through your demo, DemoRobot will measure the time it takes you to go from one stage to another
and log that information into the `Logs` you can access from the main dashboard navigation.

## Commands in Namespaces

DemoRobot has `namespaces` that hold commands that are specific to a certain web application.
Since DemoRobot is open source, you can easily [contribute your own](https://github.com/svrnm/DemoRobot/blob/main/CONTRIBUTE.md)

## Learn More

The documentation might not include all commands and all options you have with DemoRobot.

If you want to learn more about DemoRobot, the quickest way is, to [get in touch](mailto:joe.groner@datarobot.com)
